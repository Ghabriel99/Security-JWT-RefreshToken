...
public class AuthenticationService {
    ...
    public AuthenticationResponse register(RegisterRequest request) {
        ...
        return AuthenticationResponse.builder()
                .accessToken(jwtToken) //after register use the access token
    ...
    public AuthenticationResponse authenticate(AuthenticationRequest request) {
        ...
        var jwtToken = jwtService.generateToken(user);
        var refreshToken = jwtService.generateRefreshToken(user);
        revokeAllUserTokens(user);  //before save the token revolke the user's token
    ...
    public void refreshToken(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        refreshToken = authHeader.substring(7); //position 7 after space of bearer token
        userEmail =  jwtService.extractUsername(refreshToken);//to extract the userEmail from Jwt token

        if (userEmail != null ) {
            var user = this.repository.findByEmail(userEmail).orElseThrow();

            if (jwtService.isTokenValid(refreshToken, user))  { //double check istokenvalid
